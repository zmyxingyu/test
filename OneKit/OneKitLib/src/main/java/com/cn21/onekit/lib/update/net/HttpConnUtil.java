...
//					conn.connect();   //conn.getOutputStream()包括了先进行conn.connect()
	...
	/**
	 * 设置灰度环境cookie：先通过获取头信息的方式获取cookie，然后 再将获取的cookie写入下一个请求
	 * @param conn
     */
	private static void getCookie(Context context,HttpURLConnection conn) {
...
//			DefaultShared.putString(context,Constants.KEY_GET_REQUEST_COOKIES, sbu.toString());
		}
	}
